...
public class FileUtils extends CordovaPlugin {
    ...
    /**
     * use to retrieve all images
     * by Jym | 01-12-2016
     * */
    private JSONObject mediaStoreImages() throws JSONException {
        ...
        Cursor cur = this.cordova.getActivity().getContentResolver().query(imageUri, cols, MediaStore.Images.Media.DATA+" LIKE ?", new String[] {"%/DCIM/Camera/%"}, MediaStore.Images.Media.DATE_TAKEN + " DESC");
        int imageColumnIndex = cur.getColumnIndex(MediaStore.Images.Media._ID);

        JSONObject images_obj = new JSONObject();
        JSONObject ls = new JSONObject();

        images_obj.put("count", cur.getCount());
        for (int i=0; i<cur.getCount(); i++){
            cur.moveToPosition(i);
            int img_id = cur.getInt(imageColumnIndex);
            int dataColumnIndex = cur.getColumnIndex(MediaStore.Images.Media.DATA);
            ls.put("img_path_"+i, cur.getString(dataColumnIndex));
        }
        images_obj.put("list", ls);
    ...
    private JSONObject requestAllPaths() throws JSONException {
        ...
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
          try {
            ...
            //ret.put("getMediaStoreImages", mediaStoreImages());//added by jym | 01-12-2016
          }
          catch(NullPointerException e) {
